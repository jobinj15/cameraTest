...
//   };
  ...
  onApiActionDone(item, type) {
    ...
    if (!prodListStore.products || !prodListStore.products.length) return;
  ...
  modifyVariantSelection(index, subIndex) {
    ...
    for (var i = 0; i < size; i++) {
      ...
      if (i == subIndex) item.selected = true;
    ...
    for (let nItem of variants) {
      for (let nSubItem of nItem.data) {
        if (nSubItem.selected) newSelections.push(nSubItem.value_id);
      }
    }

    console.log(
      'modifyVariantSelection after ' + JSON.stringify(newSelections),
    );
    this.callVariantApi(newSelections.toString());
  }

  drawVariants(item) {
    if (!item.variants || !item.variants.length) return <View />;
                ...
                <Ripple
                  ...
                  <Text
                    style={[
                      styles.variant,
                      {
                        backgroundColor: subItem.selected
                          ? colors.PRIMARY
                          : colors.WHITE,
                        borderColor: subItem.selected
                          ? colors.BORDER
                          : colors.PRIMARY,
  ...
  render() {
    ...
    if (!prodDetStore.isApiLoaded && !prodDetStore.loading) return <View />;
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          <View>
            ...
            <View
              ...
              <Text style={[styles.bigBold]}>{item.name}</Text>
              ...
              <Text style={[styles.productKey, {marginTop: 20}]}>
                {constants.TXT_DESC}
              </Text>
  ...
  bottomView() {
    return (
      <TouchableWithoutFeedback
        ...
        <View style={[styles.bottomView, {height: 45}]}>
          <Text
            style={[
              styles.stripLabel,
  ...
  onPlusClicked() {
    if (prodDetStore.cartUpdating) return;
